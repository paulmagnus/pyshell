...
class Process:
    ...
    def set_flushed(self, _):
        self._flushed = ((self.stdout is None or
                         self.stdout.flushed) and
                         (self.stderr is None or
                          self.stderr.flushed))
    ...
    def __del__(self):
        ...
        # This might be better if it was replaced with self.run()
    ...
    def _begin_process(self):
        ...
        # print('Beginning process')
        # print('proc =', self._proc_list)
        # print('stdin =', self._stream)
        # print('stdout =', stdout)
        # print('stderr =', stderr)
        ...
        if self._stdout is not None:
            self._stdout.set_in_stream(self._proc.stdout)
        if self._stderr is not None:
            self._stderr.set_in_stream(self._proc.stderr)
